...
echo "[$HOSTNAME][0]==================="
echo " !! Now !! We are in gyolo_tune.sh @ $HOSTNAME !!"
echo "[$HOSTNAME][0]-------------------"
...
### 3_ 執行訓練命令 ###
## 超參數設定
NBatch=4    # v100 超過 254會failed
#Weight="runs/train-cap/gyolo-b128/weights/epoch59.pt" # gyolo b128
#Weight="runs/train-cap/gyolo-e-b112/weights/epoch59.pt" #gyolo-e b112
Weight="runs/train-co/gtune-b128/weights/epoch9.pt"

#TRAIN_CMD="torchrun --nproc_per_node=$NGPU --nnodes=$NNODES --node_rank=$NODE_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT \
#          caption/tune.py --device $DEVICE_LIST --epochs $NEpoch --batch-size $NBatch --workers $NWorker \
#          --weights runs/train-cap/gyolo-b128/weights/epoch59.pt \
#          --cfg models/caption/gyolo.yaml --name gtune-b128 --data data/coco.yaml \
#          --hyp data/hyps/hyp.scratch-cap.yaml --img 640 --optimizer AdamW --save-period 1"

CMD="python caption/val.py --data coco.yaml --batch $NBatch --weights $Weight --img 640 --conf 0.001 --iou 0.7 --device 0 --save-json --export-mas"
