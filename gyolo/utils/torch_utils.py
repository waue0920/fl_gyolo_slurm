...
def smart_DDP(model, find_unused_parameters = False):
    ...
    return DDP(model, device_ids = [LOCAL_RANK], output_device = LOCAL_RANK, find_unused_parameters = find_unused_parameters)
...
def smart_resume(ckpt, optimizer, optimizer2, ema=None, weights='yolov5s.pt', epochs=300, resume=True):
    ...
    if ckpt['optimizer2'] is not None:
        optimizer2.load_state_dict(ckpt['optimizer2'])  # optimizer
