...
from utils.model_utils import DET_LAYERS, find_layer
...
class ComputeLoss:
    ...
    def __init__(self, model, use_dfl=True, overlap=True):
        ...
        det_layer = find_layer(m, DET_LAYERS)
        m = m[det_layer] # Detect() module
    ...
    def __call__(self, p, targets, masks, semasks, img=None, epoch=0):
        ...
        feats_, pred_masks_, proto_, psemasks_ = p if len(p) == 4 else p[1]

        feats, pred_masks, proto, psemasks = feats_[0], pred_masks_[0], proto_[0], psemasks_[0]
        feats2, pred_masks2, proto2, psemasks2 = feats_[1], pred_masks_[1], proto_[1], psemasks_[1]
        ...
        target_labels2, target_bboxes2, target_scores2, fg_mask2, target_gt_idx2 = self.assigner(
        ...
        pt2 = torch.flatten(psemasks2, start_dim = 2).permute(0, 2, 1)
        ...
        total_loss2 = (sigmoid_focal_loss(pt2, gt, alpha = .25, gamma = 2., reduction = 'mean')) / 2.
        loss[4] += total_loss2 * 20.

        # dice loss
        pt = torch.flatten(psemasks.softmax(dim = 1))
        pt2 = torch.flatten(psemasks2.softmax(dim = 1))
        ...
        inter_mask2 = torch.sum(torch.mul(pt2, gt))
        union_mask2 = torch.sum(torch.add(pt2, gt))
        dice_coef2 = (2. * inter_mask2 + 1.) / (union_mask2 + 1.)
        loss[5] += (1. - dice_coef2) / 2.
...
class ComputeLossLH:
    ...
    def __init__(self, model, use_dfl=True, overlap=True):
        ...
        self.assigner3 = TaskAlignedAssigner(topk=int(os.getenv('YOLOM', 1)),
                                            num_classes=self.nc,
                                            alpha=float(os.getenv('YOLOA', 0.5)),
                                            beta=float(os.getenv('YOLOB', 6.0)))
        self.bbox_loss = BboxLoss(m.reg_max - 1, use_dfl=use_dfl).to(device)
    ...
    def __call__(self, p, targets, masks, semasks, img=None, epoch=0):
        ...
        target_labels3, target_bboxes3, target_scores3, fg_mask3, target_gt_idx3 = self.assigner3(
        ...
        target_scores_sum3 = target_scores3.sum()
        ...
        loss[2] += self.BCEcls(pred_scores3, target_scores3.to(dtype)).sum() / target_scores_sum3  # BCE
        ...
        # bbox loss
        if fg_mask3.sum():
            loss0_, loss3_, _ = self.bbox_loss(pred_distri3,
                                                  pred_bboxes3,
                                                  anchor_points,
...
class ComputeCaptionLoss:
    def __init__(self, model):
        device = next(model.parameters()).device  # get model device
        self.hyp = model.hyp  # hyperparameters
        self.device = device

        self.NLL = nn.NLLLoss(ignore_index = self.hyp['caption_pad_idx'])

    def __call__(self, preds, caps):
        bs = caps.shape[0]

        caps = caps[:, 1:].contiguous()
        preds = preds[:, :-1].contiguous()

        l_cap = torch.zeros(1, device = self.device)
        l_cap = torch.add(l_cap, self.NLL(preds.view(-1, self.hyp['caption_vocab_size']), caps.view(-1)))
        l_cap *= self.hyp['caption_loss_gain']

        return l_cap * bs, l_cap.detach()
