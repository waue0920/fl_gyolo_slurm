...
from utils.caption.caption_utils import create_caption_and_mask, create_src_mask
...
class RepNCSPELAN4B(nn.Module):
    ...
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = nn.Sequential(RepNCSP(c3//2, c4, c5), Conv(c4, c4, 3, 1))
        self.cv3 = nn.Sequential(RepNCSP(c4, c4, c5), Conv(c4, c4, 3, 1))
        self.cv4 = Conv(c3+(2*c4), c2//2, 1, 1)
        self.cvcap = Conv(c3+(2*c4), c2//2, 1, 1)
...
class ASCDown(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out, shortcut, kernels, groups, expand
        super().__init__()
        self.c = c2 // 2
        self.cv1 = Conv(c1 // 2, self.c, 1, 1, 0)
        self.cv2 = Conv(self.c, self.c, 3, 2, 1, g=self.c, act=False)
        self.cv3 = Conv(c1 // 2, self.c, 1, 1, 0)

    def forward(self, x):
        x = torch.nn.functional.avg_pool2d(x, 2, 1, 0, False, True)
        x1,x2 = x.chunk(2, 1)
        x1 = self.cv2(self.cv1(x1))
        x2 = torch.nn.functional.max_pool2d(x2, 3, 2, 1)
        x2 = self.cv3(x2)
        return torch.cat((x1, x2), 1)
...
class DetectMultiBackend(nn.Module):
    ...
    def warmup(self, imgsz=(1, 3, 640, 640), caption_idx = None):
        # Warmup model by running inference once
        warmup_types = self.pt, self.jit, self.onnx, self.engine, self.saved_model, self.pb, self.triton
        if any(warmup_types) and (self.device.type != 'cpu' or self.triton):
            im = torch.empty(*imgsz, dtype=torch.half if self.fp16 else torch.float, device=self.device)  # input
            if caption_idx is not None:
                _, src_mask = create_src_mask(im)
                # TODO: get length from hyp
                cap, cap_mask = create_caption_and_mask(128)
                # TODO: model key
                self.model.model[caption_idx].set_params(src_mask.to(device = self.device), cap.to(device = self.device), cap_mask.to(device = self.device))
...
class AutoShape(nn.Module):
    ...
    def __init__(self, model, verbose=True):
        ...
        self.det_layer = -1
        if self.pt:
            m = self.model.model.model if self.dmb else self.model.model
            self.det_layer = find_layer(m, DET_LAYERS)
            m = m[self.det_layer]  # Detect()
    ...
    def _apply(self, fn):
        ...
        if self.pt:
            m = self.model.model.model[self.det_layer] if self.dmb else self.model.model[self.det_layer]  # Detect()
